                ...
                <table class="table table-striped table-hover min-w-full">
                    ...
                    <tbody>
                        @forelse($transactions as $transac)
                            <tr>
                                <td>{{ $transac->date->format('d/m/Y H:i') }}</td>
                                <td>{{ number_format($transac->montant, 0, ',', ' ') }}</td>
                                <td>{{ $transac->gestionnaire_name }}</td>
                                <td>{{ $transac->reference ?? 'N/A' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="text-center py-8">Aucune transaction</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
